	...
	*crc ^= 0xFFFF;
		...
		&& (rxBuffer[(i + 1) % FRAME_BUFFER_SIZE] == ((*crc >> 8) & 0xFF))) //check CRC
	{
		...
		if(Ax25Config.allowNonAprs || (((rxBuffer[(pathEnd + 1) % FRAME_BUFFER_SIZE] == 0x03) && (rxBuffer[(pathEnd + 2) % FRAME_BUFFER_SIZE] == 0xF0))))
		{
			...
			if(rxFrameHead == rxFrameTail) // was txFrameHead — wrong pointer, caused premature full flag
...
void Ax25TransmitCheck(void)
{
	 ...
	 {
	 	// Block TX if HDLC decoder is actively decoding a frame on any demodulator,
	 	// even if modem PLL DCD has dropped momentarily (fading, bit pattern).
	 	bool rxActive = (Ax25GetRxStage(0) != RX_STAGE_IDLE) ||
	 	                (Ax25GetRxStage(1) != RX_STAGE_IDLE);

	 	if(!ModemDcdState() && !rxActive) //channel is free and no active decoding
	 	{
	 		txInitStage = TX_INIT_TRANSMITTING; //transmit right now
	 		txRetries = 0;
	 		transmitStart();
	 	}
	 	else //channel is busy or decode in progress
	 	{
