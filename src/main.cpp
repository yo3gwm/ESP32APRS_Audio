...
extern volatile unsigned long pttActiveSince;
extern volatile uint32_t adcIsrCount;
extern volatile int fifoSampleCount;
extern volatile uint32_t fifoOverflowCount;
...
// pcnt_evt_t struct removed (PCNT unused)
...
void defaultConfig()
{
    ...
    //--RF Module
    config.rf_en = false;
    config.rf_type = RF_SA8x8_VHF;
    config.freq_rx = 144.3900;
    config.freq_tx = 144.3900;
    config.freq_ppm   = 0;
    config.rf_filters = 0; // all filters off by default
    config.band = 0;
    config.sql_level = 1;
    config.rf_power = LOW;
    config.volume = 6;
...
//     config.uart0_rts_gpio = -1;
    ...
    config.fx25_mode = 1; // FX.25 RX only — TX stays standard AX.25 for maximum compatibility
}
...
static String SA8x8_checkType()
{
    if (config.rf_type == RF_SA8x8_VHF ||
        ...
        return "";
    return "ERR: SA8x8 module type not configured in memory\n"
           "Select SA8x8 in Module Type and press Apply Change first.";
}
...
// Hardware-only init: power on module and init serial — no AT commands sent.
// Called at boot so Apply Change (RF_MODULE false) can use the serial port later.
void RF_HW_INIT()
{
    if (!config.rf_en || config.rf_type == RF_NONE) return;
    if (config.rf_rx_gpio == -1 || config.rf_tx_gpio == -1 || config.rf_pd_gpio == -1) return;

    pinMode(config.rf_pwr_gpio, OUTPUT);
    digitalWrite(config.rf_pwr_gpio, !config.rf_pwr_active); // PWR OFF
    ...
    delay(1000);
    while (SerialRF.available()) SerialRF.read();             // flush startup bytes
    log_d("RF Module %s powered on — configure via Apply Change", RF_TYPE[config.rf_type]);
    Serial.printf("[RF] Module powered on. Press Apply Change to configure.\r\n");
}
...
void RF_MODULE(bool boot)
{
    ...
    if (boot)
    {
        // Full init: disable ADC, power-cycle module, init serial
        adcEn = -1;
        dacEn = -1;
        pinMode(config.rf_pwr_gpio, OUTPUT);
        digitalWrite(config.rf_pwr_gpio, !config.rf_pwr_active); // RF POWER OFF
        pinMode(config.rf_pd_gpio, OUTPUT);
        digitalWrite(config.rf_pd_gpio, !config.rf_pd_active);   // PD LOW
        delay(500);
        SerialRF.begin(config.rf_baudrate, EspSoftwareSerial::SWSERIAL_8N1, config.rf_rx_gpio, config.rf_tx_gpio);
    ...
    {
        // Reconfigure only: flush serial buffer, keep module and ADC running.
        ...
        if (pttActiveSince > 0 || hw_afsk_dac_isr)
        {
            Serial.printf("[RF] Waiting for TX to finish before module reconfigure...\r\n");
            unsigned long waitStart = millis();
            while ((pttActiveSince > 0 || hw_afsk_dac_isr) && (millis() - waitStart) < 5000)
                delay(50);
            if (pttActiveSince > 0 || hw_afsk_dac_isr)
            {
                Serial.printf("[RF] TX still active after 5s, forcing PTT off\r\n");
                setPtt(false);
                pttOff = false;
            }
        }

        // Block new TX for the duration of the AT command session
        AFSK_SetTxInhibit(true);
    ...
    // PPM correction: corrected = nominal × (1 + ppm/1e6)
    float rf_freq = config.freq_rx * (1.0f + config.freq_ppm / 1000000.0f);
    ...
    else if ((config.rf_type == RF_SR_2WVS) || (config.rf_type == RF_SR_2WUS))
    {
        ...
        // CTCSS always disabled for APRS — 0xFF 0xFF = off
        String tone_rx = hexToString("FF") + hexToString("FF");
        String tone_tx = hexToString("FF") + hexToString("FF");

        if (config.band == 0)
            flag |= 0x02;
        if (config.rf_power == 0)
            flag1 |= 0x02;
        sprintf(str, "AT+DMOGRP=%0.5f,%0.5f,%s,%s,%d,%d\r\n", rf_freq, rf_freq, tone_rx.c_str(), tone_tx.c_str(), flag, flag1);
    ...
    if (boot)
    {
        // Re-enable ADC only if we disabled it at the start (boot path only)
        adcEn = 1;
        dacEn = -1;
    }
    else
    {
        // Release TX inhibit set at the start of the Apply reconfigure path
        AFSK_SetTxInhibit(false);
    }
}
...
// ── CPU load via idle hooks ──────────────────────────────────────────────────
static volatile uint32_t s_idle_count[2] = {0, 0};
static uint32_t          s_idle_snap[2]  = {0, 0};
static uint32_t          s_idle_max[2]   = {1, 1}; // self-calibrates upward
static uint8_t           s_cpu_load[2]   = {0, 0};
static uint32_t          s_load_ts       = 0;

static bool IRAM_ATTR cpu_idle_hook0(void) { s_idle_count[0] = s_idle_count[0] + 1; return false; }
static bool IRAM_ATTR cpu_idle_hook1(void) { s_idle_count[1] = s_idle_count[1] + 1; return false; }

void cpu_load_init(void)
{
    esp_register_freertos_idle_hook_for_cpu(cpu_idle_hook0, 0);
    esp_register_freertos_idle_hook_for_cpu(cpu_idle_hook1, 1);
}

void cpu_load_update(void)
{
    uint32_t now = millis();
    if (now - s_load_ts < 2000) return; // update every 2 s
    s_load_ts = now;
    for (int i = 0; i < 2; i++) {
        uint32_t idle = s_idle_count[i] - s_idle_snap[i];
        s_idle_snap[i] = s_idle_count[i];
        if (idle > s_idle_max[i]) s_idle_max[i] = idle; // track max = 100% idle
        s_cpu_load[i] = (uint8_t)(100 - min((uint32_t)100, idle * 100 / s_idle_max[i]));
    }
}

uint8_t getCpuLoad(int core) { return (core < 2) ? s_cpu_load[core] : 0; }
// ────────────────────────────────────────────────────────────────────────────
...
void setup()
{
    ...
    Serial.println();
    Serial.println("Start ESP32APRS_Audio V" + String(VERSION) + String(VERSION_BUILD) + "_" + String(BUILD_TIMESTAMP));
...
// display.begin(SSD1306_SWITCHCAPVCC, 0x3C, false); // initialize with the I2C addr 0x3C (for the 128x64)
    ...
    log_d("RF Module hardware init");
    delay(10);

    if (config.rf_en)
        RF_HW_INIT(); // power on + serial init only — AT commands sent only on Apply Change
    ...
    cpu_load_init(); // register idle hooks before tasks start
    log_d("Start Task");
#ifdef __XTENSA__
    ...
    xTaskCreatePinnedToCore(
        ...
        4096,                /* Stack size in words (increased: gap detector uses Serial.printf) */
        NULL,                /* Task input parameter */
        12,                  /* Above async_tcp(10): audio must never be starved by web server */
...
#endif
        NULL,            /* Task input parameter */
        11,              /* Above async_tcp(10): packet processing not blocked by web server */
...
void taskAPRS(void *pvParameters)
{
    ...
    for (;;)
    {
        ...
        // PTT watchdog: force off if TX exceeds 10 seconds (TOT)
        if (pttActiveSince > 0 && (millis() - pttActiveSince) > 10000)
        {
            Serial.printf("[PTT-WDG] TX timeout %lums — forcing PTT off!\r\n", millis() - pttActiveSince);
            setPtt(false);
            pttOff = false;
        }
        long now = millis();
            ...
            if (Ax25ReadNextRxFrame(&buf, &size, &peak, &valley, &signalLevel, &fixed, &mV))
            {
                ...
                if (packet2Raw(tnc2, incomingPacket) > 0)
                {
                    ...
                    if (!(type & FILTER_THIRDPARTY))
                    {
                        ...
                        // Object packets: key by object name so they get a separate pkgList
                        // slot from the source station's regular beacons.
                        // The web UI still shows "OBJ(SRC)" because it re-parses raw.
                        if (type & FILTER_OBJECT) {
                            const char *colon = strchr(tnc2.c_str(), ':');
                            if (colon && *(colon + 1) == ';') {
                                int last_nonspace = -1;
                                for (int z = 0; z < 9; z++) {
                                    char c = *(colon + 2 + z);
                                    if ((uint8_t)c < 0x20 || (uint8_t)c > 0x7E) break;
                                    if (c != ' ') last_nonspace = z;
                                }
                                if (last_nonspace >= 0) {
                                    memset(call, 0, sizeof(call));
                                    for (int z = 0; z <= last_nonspace && z < 10; z++)
                                        call[z] = *(colon + 2 + z);
                                }
                            }
                        }
                        ...
                        if (rawP)
                        {
                            ...
                            // toCharArray(buf, len) copies only len-1 chars — use memcpy
                            memcpy(rawP, tnc2.c_str(), tnc2.length());
                    ...
                    else if (type & FILTER_THIRDPARTY)
                    {
                        // Extract inner packet from third-party frame and store it separately
                        int cb = tnc2.indexOf(":}");
                        if (cb >= 0)
                        {
                            String inner = tnc2.substring(cb + 2); // YO8RXT-15>APOBJ,...:;...
                            int ig = inner.indexOf('>');
                            if (ig > 0 && ig < 11)
                            {
                                char inner_call[11] = {0};
                                strncpy(inner_call, inner.c_str(), ig);
                                uint16_t inner_type = type & ~FILTER_THIRDPARTY;
                                char *rawP = (char *)calloc(inner.length() + 1, sizeof(char));
                                if (rawP)
                                {
                                    memcpy(rawP, inner.c_str(), inner.length());
                                    pkgListUpdate(inner_call, rawP, inner_type, 0, incomingPacket.mVrms);
                                    handle_ws(rawP, inner.length(), incomingPacket.mVrms);
                                    free(rawP);
                                }
                            }
                        }
            ...
            if (newDigiPkg)
            {
                ...
                if ((type & config.digiFilter))
                {
                    ...
                    if (dlyFlag > 0)
                    {
                        ...
                        log_d("DIGI_REPEAT[%dms]: %s", digiDelay, digiPkg.c_str());
...
void wifiConnection()
{
    ...
    // Wait for any ongoing TX to finish before AFSK_deinit.
    // Radio has priority over WiFi reconnect — never interrupt a transmission.
    // If TX doesn't finish within 5s (watchdog handles it separately), proceed anyway.
    if (pttActiveSince > 0)
    {
        Serial.printf("[WiFi] Waiting for TX to finish before reconnect...\r\n");
        unsigned long waitStart = millis();
        while (pttActiveSince > 0 && (millis() - waitStart) < 5000)
            delay(50);
        if (pttActiveSince > 0)
        {
            // TX still stuck after 5s — force off as last resort
            Serial.printf("[WiFi] TX still active after 5s, forcing PTT off\r\n");
            setPtt(false);
            pttOff = false;
        }
        else
        {
            Serial.printf("[WiFi] TX finished, proceeding with reconnect\r\n");
        }
    }

    // Stop ADC ISR before WiFi mode changes to prevent stale/corrupt samples.
    // RF activity during WiFi init causes analogReadMilliVolts to return near-constant
    // values, filling the FIFO with flat data → mVrms≈2, dcd=0, gap for minutes.
    if (AFSKInitAct)
        AFSK_TimerEnable(false);

    // disconnect(wifioff=false) to avoid double esp_wifi_deinit with WiFi.mode(WIFI_OFF).
    // eraseap=true clears stored AP; wifioff=false keeps netif consistent.
    WiFi.disconnect(false, true);
    WiFi.persistent(false);
    WiFi.mode(WIFI_OFF); // Switch WiFi off — single clean deinit

    wifiTTL = 0;
    delay(1500); // allow netif teardown to complete before re-init
    ...
    // Restart ADC ISR with clean FIFO after WiFi operation.
    // AFSK_TimerEnable(true) calls RingBuffer_Init → discards any stale/corrupt samples
    // accumulated during WiFi mode changes. Recovery in <40ms regardless of connect result.
    if (AFSKInitAct)
        AFSK_TimerEnable(true);
}
