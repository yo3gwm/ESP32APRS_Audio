...
extern uint8_t getCpuLoad(int core);
extern String SA8x8_getVERSION();
...
void setMainPage(AsyncWebServerRequest *request)
{
	...
	strcat(webString, "<td style=\"color:#000080;font-size:9pt;text-align:left;white-space:normal;word-break:break-word;max-width:30ch;\">${row.wx || ''}</td>\n");
	strcat(webString, "`;\n");
	...
	strcat(webString, "ESP32APRS_Audio Web Configuration<br />Copy right ©2026.\n");
	strcat(webString, "<br />\n");
	strcat(webString, "</div>\n");
	strcat(webString, "</div>\n");
...
void handle_dashboard(AsyncWebServerRequest *request)
{
	...
	strcat(webString, "<div id=\"lastHeard\" style=\"overflow-x:auto;width:100%;\">\n");
	...
	strcat(webString, "<th style=\"min-width:35ch\">WX/TEXT</th>\n");
...
void handle_rfver(AsyncWebServerRequest *request)
{
	String ver = SA8x8_getVERSION();
	request->send(200, "text/plain", ver);
}

void handle_rfwrite_group(AsyncWebServerRequest *request)
{
	request->send(200, "text/plain", RF_SA8x8_WriteGroup());
}

void handle_rfwrite_volume(AsyncWebServerRequest *request)
{
	request->send(200, "text/plain", RF_SA8x8_WriteVolume());
}
...
void handle_sysinfo(AsyncWebServerRequest *request)
{
	...
	strcat(html, "<th><span>CPU Load</span></th>\n");
	...
	snprintf(temp_buffer, sizeof(temp_buffer), "<td><b>C0:%d%% C1:%d%%</b></td>\n", getCpuLoad(0), getCpuLoad(1));
	strcat(html, temp_buffer);
				...
				if (aprsParse.parse_aprs(&aprs))
				{
					...
					if (aprs.packettype & T_WX) {
						...
						if (aprs.wx_report.flags & W_TEMP) {
							snprintf(wx_tmp, sizeof(wx_tmp), "T:%.1fC ", aprs.wx_report.temp);
							strncat(wx_str, wx_tmp, sizeof(wx_str) - strlen(wx_str) - 1);
						}
						if (aprs.wx_report.flags & W_HUM) {
							snprintf(wx_tmp, sizeof(wx_tmp), "H:%d%% ", (int)aprs.wx_report.humidity);
							strncat(wx_str, wx_tmp, sizeof(wx_str) - strlen(wx_str) - 1);
						}
						if (aprs.wx_report.flags & W_BAR) {
							snprintf(wx_tmp, sizeof(wx_tmp), "P:%.0fmb ", aprs.wx_report.pressure);
							strncat(wx_str, wx_tmp, sizeof(wx_str) - strlen(wx_str) - 1);
						}
						if (aprs.wx_report.flags & W_WS) {
							snprintf(wx_tmp, sizeof(wx_tmp), "W:%.0fkm/h", aprs.wx_report.wind_speed);
							strncat(wx_str, wx_tmp, sizeof(wx_str) - strlen(wx_str) - 1);
						}
					} else if (aprs.comment && aprs.comment_len > 0) {
						int wi = 0;
						for (unsigned int ci = 0; ci < aprs.comment_len && wi < (int)sizeof(wx_str) - 2; ci++) {
							char c = aprs.comment[ci];
							if (c == '"' || c == '\\') {
								wx_str[wi++] = '\\';
								if (wi < (int)sizeof(wx_str) - 1) wx_str[wi++] = c;
...
void handle_radio(AsyncWebServerRequest *request)
{
	...
	{
		config.rf_filters = 0; // reset before reading checkboxes — unchecked boxes send nothing
		for (uint8_t i = 0; i < request->args(); i++)
		{
				...
				if (request->argName(i) == "freq")
			{
				if (request->arg(i) != "")
				{
					if (isValidNumber(request->arg(i)))
					{
						...
						config.freq_tx = config.freq_rx;
					}
				}
			}
			if (request->argName(i) == "freq_ppm")
			{
				if (request->arg(i) != "")
				{
					if (isValidNumber(request->arg(i)))
						config.freq_ppm = request->arg(i).toInt();
				}
			}

			if (request->argName(i) == "rf_filter_pre")
			{
				if (request->arg(i) == "OK") config.rf_filters |= 0x01;
			}
			if (request->argName(i) == "rf_filter_hp")
			{
				if (request->arg(i) == "OK") config.rf_filters |= 0x02;
			}
			if (request->argName(i) == "rf_filter_lp")
			{
				if (request->arg(i) == "OK") config.rf_filters |= 0x04;
			}
		...
		for (uint8_t i = 0; i < request->args(); i++)
		{
			...
			if (request->argName(i) == "preamble")
			{
				if (request->arg(i) != "")
				{
					if (isValidNumber(request->arg(i)))
					{
						int ms = request->arg(i).toInt();
						if (ms < 100) ms = 100;
						if (ms > 5000) ms = 5000;
						config.preamble = ms / 100;
					}
				}
			}
	...
	else
	{
		...
		strcat(html, "if(type==4||type==7){document.getElementById(\"freq\").setAttribute(\"min\",136);};\n");
		strcat(html, "if(type==2||type==5||type==8){document.getElementById(\"freq\").setAttribute(\"max\",470);};\n");
		strcat(html, "if(type==2||type==5||type==8){document.getElementById(\"freq\").setAttribute(\"min\",400);};\n");
		strcat(html, "if(type==3){document.getElementById(\"freq\").setAttribute(\"min\",320);};\n");
		...
		// disable all inputs/selects/type=button in the form, except radioEnable and the hidden field
		...
		if (config.rf_en)
		{
			strcat(html, "<td style=\"text-align: left;\"><label class=\"switch\"><input type=\"checkbox\" id=\"radioEnableChk\" name=\"radioEnable\" value=\"OK\" checked onchange=\"rfEnableToggle()\"><span class=\"slider round\"></span></label> <small>(only for SA818 / SA8x8 / SR110 / SR120 / SR350)</small></td>\n");
		}
		else
		{
			strcat(html, "<td style=\"text-align: left;\"><label class=\"switch\"><input type=\"checkbox\" id=\"radioEnableChk\" name=\"radioEnable\" value=\"OK\" onchange=\"rfEnableToggle()\"><span class=\"slider round\"></span></label> <small>(only for SA818 / SA8x8 / SR110 / SR120 / SR350)</small></td>\n");
		}
		strcat(html, "</tr>\n");
		strcat(html, "<tr>\n");
		...
		strcat(html, "</select>\n");
		// Version button — visible only for SA8x8 types (1, 2, 3)
		{
			bool isSA8x8 = (config.rf_type == RF_SA8x8_VHF ||
			                config.rf_type == RF_SA8x8_UHF ||
			                config.rf_type == RF_SA8x8_350);
			snprintf(temp_buffer, sizeof(temp_buffer),
			    " <button type=\"button\" class=\"button\" id=\"btn_rfver\" onclick=\"checkRfVersion()\" style=\"display:%s;\">Check Version</button>\n",
			    isSA8x8 ? "inline" : "none");
			strcat(html, temp_buffer);
		}
		strcat(html, "</td>\n");
		...
		snprintf(temp_buffer, sizeof(temp_buffer), "<td style=\"text-align: left;\"><input type=\"number\" id=\"freq\" name=\"freq\" min=\"%.4f\" max=\"%.4f\"\n", freqMin, freqMax);
		strcat(html, temp_buffer);
		...
		strcat(html, "</tr>\n");
		strcat(html, "<tr>\n");
		strcat(html, "<td align=\"right\"><b>Freq Correction:</b></td>\n");

		snprintf(temp_buffer, sizeof(temp_buffer), "<td style=\"text-align: left;\"><input type=\"number\" id=\"freq_ppm\" name=\"freq_ppm\" min=\"-999\" max=\"999\" step=\"1\" value=\"%d\" /> ppm</td>\n", config.freq_ppm);
		strcat(html, temp_buffer);
		...
		strcat(html, "</tr>\n");
		strcat(html, "<tr>\n");
		strcat(html, "<td align=\"right\"><b>Pre/De-emphasis:</b></td>\n");
		if (config.rf_filters & 0x01)
			strcat(html, "<td style=\"text-align: left;\"><label class=\"switch\"><input type=\"checkbox\" name=\"rf_filter_pre\" value=\"OK\" checked><span class=\"slider round\"></span></label></td>\n");
		else
			strcat(html, "<td style=\"text-align: left;\"><label class=\"switch\"><input type=\"checkbox\" name=\"rf_filter_pre\" value=\"OK\" ><span class=\"slider round\"></span></label></td>\n");
		strcat(html, "</tr>\n");
		strcat(html, "<tr>\n");
		strcat(html, "<td align=\"right\"><b>High Pass Filter:</b></td>\n");
		if (config.rf_filters & 0x02)
			strcat(html, "<td style=\"text-align: left;\"><label class=\"switch\"><input type=\"checkbox\" name=\"rf_filter_hp\" value=\"OK\" checked><span class=\"slider round\"></span></label></td>\n");
		else
			strcat(html, "<td style=\"text-align: left;\"><label class=\"switch\"><input type=\"checkbox\" name=\"rf_filter_hp\" value=\"OK\" ><span class=\"slider round\"></span></label></td>\n");
		strcat(html, "</tr>\n");
		strcat(html, "<tr>\n");
		strcat(html, "<td align=\"right\"><b>Low Pass Filter:</b></td>\n");
		if (config.rf_filters & 0x04)
			strcat(html, "<td style=\"text-align: left;\"><label class=\"switch\"><input type=\"checkbox\" name=\"rf_filter_lp\" value=\"OK\" checked><span class=\"slider round\"></span></label></td>\n");
		else
			strcat(html, "<td style=\"text-align: left;\"><label class=\"switch\"><input type=\"checkbox\" name=\"rf_filter_lp\" value=\"OK\" ><span class=\"slider round\"></span></label></td>\n");
		strcat(html, "</tr>\n");
		strcat(html, "<tr>\n");
		strcat(html, "<td colspan=\"2\" style=\"text-align:justify;\">\n");
		{
			const char *d = (config.rf_type == RF_SA8x8_VHF ||
			                 config.rf_type == RF_SA8x8_UHF ||
			                 config.rf_type == RF_SA8x8_350) ? "inline" : "none";
			strcat(html, "<button class=\"button\" type='button' id='btnWrGroup' style=\"display:");
			strcat(html, d);
			strcat(html, ";\" onclick=\"rfWriteCmd('/rfwritegroup','Write Group')\">Write Group</button> ");
			strcat(html, "<button class=\"button\" type='button' id='btnWrVolume' style=\"display:");
			strcat(html, d);
			strcat(html, ";\" onclick=\"rfWriteCmd('/rfwritevolume','Write Volume')\">Write Volume</button> ");
		...
		strcat(html, "</td>\n");
		strcat(html, "</tr></table><br />\n");
		strcat(html, "</form>");
		strcat(html, "<script>rfEnableToggle();rfType();</script>\n");
		...
		strcat(html, "</tr>\n");
		strcat(html, "<tr>\n");
		strcat(html, "<td align=\"right\"><b>SlotTime:</b></td>\n");
		...
		strcat(html, "</tr>\n");
		strcat(html, "<tr>\n");
		strcat(html, "<td align=\"right\"><b>TxDelay:</b></td>\n");
		snprintf(temp_buffer, sizeof(temp_buffer),
			"<td style=\"text-align: left;\"><input type=\"number\" name=\"preamble\" min=\"100\" max=\"5000\" step=\"100\" value=\"%d\" /> mSec.</td>\n",
			config.preamble * 100);
		strcat(html, temp_buffer);
		strcat(html, "</tr>\n");
...
void handle_mod(AsyncWebServerRequest *request)
{
	...
	else if (request->hasArg("commitRF"))
	{
		for (uint8_t i = 0; i < request->args(); i++)
		{
			...
			if (request->argName(i) == "rxlog")
			{
				config.rx_log_enable = (request->arg(i) == "1");
				afskSetRxLog(config.rx_log_enable);
			}
		}
		saveConfiguration("/default.cfg", config);
		String html = "OK";
	...
	else
	{
		...
		strcat(html, "<tr>\n");
		strcat(html, "<td align=\"right\"><b>RX Log (Serial):</b></td>\n");
		strcat(html, "<td style=\"text-align: left;\">\n");
		strcat(html, "<select name=\"rxlog\" id=\"rxlog\">\n");
		if (config.rx_log_enable)
		{
			strcat(html, "<option value=\"0\">OFF</option>\n");
			strcat(html, "<option value=\"1\" selected>ON</option>\n");
		}
		else
		{
			strcat(html, "<option value=\"0\" selected>OFF</option>\n");
			strcat(html, "<option value=\"1\">ON</option>\n");
		}
		strcat(html, "</select> &nbsp;<span style=\"color:gray;font-size:9pt\">[RX] offset mVrms dcd fifo adcISR</span>\n");
		strcat(html, "</td>\n");
		strcat(html, "</tr>\n");
...
void handle_about(AsyncWebServerRequest *request)
{
	...
	if (temp_str)
	{
		snprintf(temp_str, 512, "<tr><td align=\"right\"><b>Firmware Version: </b></td><td align=\"left\"> V%s%c_%s</td></tr>\n", VERSION, VERSION_BUILD, BUILD_TIMESTAMP);
...
void webService()
{
	...
	async_server.on("/rfver", HTTP_GET, [](AsyncWebServerRequest *request)
					{ handle_rfver(request); });
	async_server.on("/rfwritegroup", HTTP_GET, [](AsyncWebServerRequest *request)
					{ handle_rfwrite_group(request); });
	async_server.on("/rfwritevolume", HTTP_GET, [](AsyncWebServerRequest *request)
