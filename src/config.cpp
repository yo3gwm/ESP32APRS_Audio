...
bool saveConfiguration(const char *filename, const Configuration &config)
{
    ...
    doc["rfFreq"]    = config.freq_rx;
    doc["rfFreqPPM"] = config.freq_ppm;
    doc["rfFilters"] = config.rf_filters;
    ...
    doc["rxLogEn"] = config.rx_log_enable;
...
bool loadConfiguration(const char *filename, Configuration &config)
{
    ...
    if (f)
    {
        ...
        config.freq_rx  = doc["rfFreq"] | config.freq_rx;
        config.freq_tx  = config.freq_rx;
        config.freq_ppm  = doc["rfFreqPPM"]  | 0;
        config.rf_filters = doc["rfFilters"] | 0;
        ...
        config.rx_log_enable = doc["rxLogEn"] | false;          
